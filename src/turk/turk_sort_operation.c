...
/*   Updated: 2025/03/03 18:47:02 by zlee             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	determine_node_steps(t_list *sa, t_list **sb)
{
	...
	if (find_middle(sa, *sb, &target1, &target2))
	 {
		determine_rotate_mid(sa, sb, &target1, &target2);
	 }
	else if (find_small(sa, sb, &target1))
	{
		determine_rotate_small(sa, sb, &target1, &target2);
	}
	else if (find_large(sa, sb, target1))
	 	determine_rotate_big(sa, sb, &target1, &target2);
	return ;
}
